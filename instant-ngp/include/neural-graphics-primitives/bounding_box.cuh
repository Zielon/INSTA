...
struct BoundingSphere {
    NGP_HOST_DEVICE BoundingSphere(){};
    NGP_HOST_DEVICE BoundingSphere(float r, const Eigen::Vector3f& p): m_r(r), m_p(p){};

    float m_r{};
    Eigen::Vector3f m_p{};

    NGP_HOST_DEVICE bool contains(const Eigen::Vector3f& p) {
        auto d = (p - m_p).norm();
        return d < m_r;
    }

    NGP_HOST_DEVICE Eigen::Vector2f ray_intersect(const Eigen::Vector3f& orig, const Eigen::Vector3f& dir) {
        auto r2 = m_r * m_r;
        auto L = m_p - orig;
        float tca = L.dot(dir);
        if (tca < 0) return { std::numeric_limits<float>::max(), std::numeric_limits<float>::max() };
        float d2 = L.dot(L) - tca * tca;
        if (d2 > r2) return { std::numeric_limits<float>::max(), std::numeric_limits<float>::max() };
