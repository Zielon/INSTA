...
template <typename T> class NerfNetworkMapper;
...
public:
	...
	virtual void load_training_data(const std::string& data_path);
    virtual void reload_training_data(bool force = false, std::string mode = "train"){};

    virtual tcnn::GPUMatrix<float> surface_closest_point(const tcnn::GPUMatrix<float>& coords, cudaStream_t stream) { return {}; };
    virtual tcnn::GPUMatrix<tcnn::network_precision_t, tcnn::RM> point_density_flame_closest_point(const tcnn::GPUMatrix<float>& points, const tcnn::GPUMatrix<tcnn::network_precision_t, tcnn::RM>& density, float radius, cudaStream_t stream) { return {}; };
    virtual tcnn::GPUMatrix<float> render_deformed(const tcnn::GPUMatrix<float>& coords, cudaStream_t stream) { return {}; };
	...
	public:
		...
		uint32_t trace(
			...
			const BoundingBox& render_aabb,
			const BoundingSphere& render_bsphere,
			const Eigen::Matrix3f& render_aabb_to_local,
	...
	void update_density_grid_flame(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream);
	...
	struct Nerf {
		...
		struct Training {
            ...
            bool penalize_density_alpha = false;
			float depth_supervision_lambda = 1.5f;
			float seg_mask_supervision_lambda = 1.f;
			float beta = 0.f;
	...
	float m_render_radius = 0.085f;
	BoundingSphere m_render_bsphere;
...
//	Eigen::Array4f m_background_color = {89.f / 255.f, 168.f / 255.f, 41.f / 255.f, 1.f};
	Eigen::Array4f m_background_color = {1.f, 1.f, 1.f, 1.f};
	...
	bool m_snap_to_pixel_centers = true;
	...
	default_rng_t m_rng;
    default_rng_t m_random_generator;
    ...
    tcnn::GPUMemory<ngp::TriangleBvhNode*> m_bvh_ptrs_gpu;
