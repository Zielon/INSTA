...
struct Triangle {
    ...
    NGP_HOST_DEVICE Eigen::Quaternionf tbn_quat() const {
        Eigen::Matrix3f R = tbn();
...
//    NGP_HOST_DEVICE Eigen::Vector3f interpolate_normal(const Eigen::Vector3f& p) const {
...
//        float u = 1.0f - v - w;
//
//        return (na * w + nb * u + nc * v).normalized();
//    }
...
//    NGP_HOST_DEVICE Eigen::Vector3f closest_normal(Eigen::Vector3f point) const {
//        point -= normal().dot(point - a) * normal();
//
//        if (point_in_triangle(point)) {
//            return interpolate_normal(point);
//        }
//
//        Eigen::Vector3f c1 = closest_point_to_line(a, b, point);
//        Eigen::Vector3f c2 = closest_point_to_line(b, c, point);
...
//
//        if (min == mag1) {
//            float t = (c1 - a).norm() / (a - b).norm();
//            return na * t + nb * (1 - t);
//        } else if (min == mag2) {
//            float t = (c2 - b).norm() / (b - c).norm();
//            return nb * t + nc * (1 - t);
//        } else {
//            float t = (c3 - c).norm() / (c - a).norm();
//            return nc * t + na * (1 - t);
//        }
//    }
