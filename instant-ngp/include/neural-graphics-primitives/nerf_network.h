...
std::string to_string_with_precision(const T a_value, const int n = 2) {
    ...
    out.precision(n);
    out << std::fixed << std::setw(n) << std::setfill('0') << a_value;
    return out.str();
}

template <typename T>
__global__ void extract_expressions(
        const uint32_t n_elements,
...
//    const uint32_t elem_idx = i / num_exp;
//    const uint32_t dim_idx = i - elem_idx * num_exp;
//    output[elem_idx * output_stride + dim_idx] = input[elem_idx * input_stride + dim_idx];
...
template <typename T>
__global__ void extract_expressions_random(
        const uint32_t n_elements,
        const uint32_t num_frames,
        const uint32_t num_exp,
        float* extra_dims_gpu,
    ...
    const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
    if (i >= n_elements) return;
    rng.advance(i * num_frames);
    auto r = ngp::random_val(rng);
    const float* extra_dims = nullptr;
//    if (extra_dims_gpu)
//        extra_dims = extra_dims_gpu + r * num_exp;
    for (int j = 0; j < num_exp; ++j) {
//        output[i + stride * j] = extra_dims[j];
        output[i + stride * j] = 1.f;
    }
}
...
public:
	...
	NerfNetwork(uint32_t n_pos_dims, uint32_t n_dir_dims, uint32_t n_extra_dims, uint32_t dir_offset, const json& pos_encoding, const json& dir_encoding, const json& density_network, const json& rgb_network, const json& mapper_network) : m_n_pos_dims{n_pos_dims}, m_n_dir_dims{n_dir_dims}, m_dir_offset{dir_offset}, m_n_extra_dims{n_extra_dims} {
		...
		local_density_network_config["n_input_dims"] = m_pos_encoding->padded_output_width() + m_n_extra_dims;
    ...
    virtual uint32_t get_mapper_output() { return 0; }
	...
	void inference_mixed_precision_impl(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>& output, bool use_inference_params = true) override {
		...
		tcnn::GPUMatrixDynamic<T> density_network_input{m_pos_encoding->padded_output_width() + m_n_extra_dims, batch_size, stream, m_pos_encoding->preferred_output_layout()};
        ...
        if (m_n_extra_dims > 0) {
            auto exp_input = input.slice_rows(input.stride() - m_n_extra_dims, m_n_extra_dims);
            auto exp_output = density_network_input.slice_rows(m_pos_encoding->padded_output_width(), m_n_extra_dims);
            tcnn::linear_kernel(extract_expressions<T>, 0, stream,
                                batch_size,
                                m_n_extra_dims,
                                exp_input.stride(),
                                exp_output.stride(),
...
//        out.close();
	...
	std::unique_ptr<tcnn::Context> forward_impl(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>* output = nullptr, bool use_inference_params = false, bool prepare_input_gradients = false) override {
		...
		forward->density_network_input = tcnn::GPUMatrixDynamic<T>{m_pos_encoding->padded_output_width() + m_n_extra_dims, batch_size, stream, m_pos_encoding->preferred_output_layout()};
        ...
        if (m_n_extra_dims > 0) {
            auto exp_input = input.slice_rows(input.stride() - m_n_extra_dims, m_n_extra_dims);
            auto exp_output = forward->density_network_input.slice_rows(m_pos_encoding->padded_output_width(), m_n_extra_dims);
            tcnn::linear_kernel(extract_expressions<T>, 0, stream,
                                batch_size,
                                m_n_extra_dims,
                                exp_input.stride(),
	...
	void backward_impl(
		...
		if (m_dir_encoding->n_params() > 0 || dL_dinput) {
			...
			m_dir_encoding->backward(
				...
				*forward.dir_encoding_ctx,
				input.slice_rows(m_dir_offset, m_dir_encoding->input_width()),
				forward.rgb_network_input.slice_rows(m_density_network->padded_output_width() + m_n_extra_dims, m_dir_encoding->padded_output_width()),
		...
		if (m_pos_encoding->n_params() > 0 || dL_dinput) {
			dL_ddensity_network_input = tcnn::GPUMatrixDynamic<T>{m_pos_encoding->padded_output_width() + m_n_extra_dims, batch_size, stream, m_pos_encoding->preferred_output_layout()};
		}
	...
	virtual void density(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>& output, default_rng_t rng, bool use_inference_params = true, float* extra_dims_gpu = nullptr, uint32_t num_frames = 0) {
	...
	virtual std::unique_ptr<tcnn::Context> density_forward(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>* output = nullptr, bool use_inference_params = false, bool prepare_input_gradients = false) {
		if (input.layout() != tcnn::CM) {
