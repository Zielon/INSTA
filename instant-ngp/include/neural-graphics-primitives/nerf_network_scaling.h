...
template <typename T>
__global__ void scale_pos_encoding(
    ...
    for (int j = 0; j < num_exp; ++j) {
        output[i + output_stride * j] = float(output[i + output_stride * j]) * float(input[i * input_stride + j]);
    }
}

template <typename T>
__global__ void scale_pos_encoding_random(
        const uint32_t n_elements,
...
public:
	...
	void inference_mixed_precision_impl(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>& output, bool use_inference_params = true) override {
        ...
        if (m_n_extra_dims > 0) {
            ...
            tcnn::linear_kernel(scale_pos_encoding<T>, 0, stream,
                                batch_size,
                                m_n_extra_dims,
                                exp_input.stride(),
                                pos_out.stride(),
    ...
    void density(cudaStream_t stream, const tcnn::GPUMatrixDynamic<float>& input, tcnn::GPUMatrixDynamic<T>& output, default_rng_t rng, bool use_inference_params = true, float* extra_dims_gpu = nullptr, uint32_t num_frames = 0) override {
        ...
        if (m_n_extra_dims > 0) {
            ...
            tcnn::linear_kernel(scale_pos_encoding_random<T>, 0, stream,
                                batch_size,
                                num_frames,
                                m_n_extra_dims,
                                extra_dims_gpu,
