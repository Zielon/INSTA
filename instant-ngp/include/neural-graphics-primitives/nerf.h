...
struct NerfDeformedPosition {
    NGP_HOST_DEVICE NerfDeformedPosition(const Eigen::Vector3f& pos): p{pos}{}
    Eigen::Vector3f p;
};
...
struct NerfCoordinate {
	NGP_HOST_DEVICE NerfCoordinate(const Eigen::Vector3f& pos, const Eigen::Vector3f& dir, float dt) : pos{pos, dt}, dt{dt}, dir{dir, dt}, deformed_pos{pos} {}
	NGP_HOST_DEVICE void set_with_optional_extra_dims(const Eigen::Vector3f& pos, const Eigen::Vector3f& dir, float dt, const float* extra_dims, uint32_t stride_in_bytes) {
		...
		this->deformed_pos = NerfDeformedPosition(pos);
	...
	NerfPosition pos;
	float dt{};
    float holder{}; // frame_id and depth
    NerfDeformedPosition deformed_pos;
	NerfDirection dir;
};
