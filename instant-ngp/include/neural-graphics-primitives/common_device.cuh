...
inline NGP_HOST_DEVICE Eigen::Array4i read_seg_mask(Eigen::Vector2f pos, const Eigen::Vector2i& resolution, const void* pixels, EImageDataType image_data_type, uint32_t img = 0) {
    Eigen::Vector2i px = image_pos(pos, resolution);
    uint8_t val[4];
    *(uint32_t*)&val[0] = ((uint32_t*)pixels)[pixel_idx(px, resolution, img)];
    return Eigen::Array4i{(int)val[0],(int)val[1],(int)val[2],0};
}
