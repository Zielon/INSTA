...
__global__ void raytrace_kernel(uint32_t n_elements, Vector3f* __restrict__ positions, Vector3f* __restrict__ directions, const TriangleBvhNode* __restrict__ nodes, const Triangle* __restrict__ triangles, const SdfPayload* __restrict__ payloads, bool apply_depth_mask);
...
public:
	...
	void ray_trace_gpu(uint32_t n_elements, Vector3f* gpu_positions, Vector3f* gpu_directions, SdfPayload* gpu_payloads, const Triangle* gpu_triangles, cudaStream_t stream) override {
#ifdef NGP_OPTIX
//		if (m_optix.available) {
...
__global__ void raytrace_kernel(uint32_t n_elements, Vector3f* __restrict__ positions, Vector3f* __restrict__ directions, const TriangleBvhNode* __restrict__ nodes, const Triangle* __restrict__ triangles, const SdfPayload* __restrict__ payloads, bool apply_depth_mask) {
	...
	auto orig = positions[i];
	auto dir = directions[i];
    auto payload = payloads[i];

	auto p = TriangleBvh4::ray_intersect(orig, dir, nodes, triangles);
	positions[i] = orig + p.second * dir;
    directions[i] = Vector3f::Constant(0.f);

	if (p.first >= 0) {
        auto tri = triangles[p.first];
        directions[i] = tri.normal();
	}
}
