...
void Testbed::reset_camera() {
	...
	m_camera <<
		...
		0.0f, -1.0f, 0.0f, 0.5f,
		0.0f, 0.0f, -1.0f, 0.5f;
	m_camera.col(3) -= m_scale * view_dir() * MESH_SCALE;
...
void Testbed::imgui() {
	...
	if (ImGui::CollapsingHeader("Training", m_training_data_available ? ImGuiTreeNodeFlags_DefaultOpen : 0)) {
		...
		ImGui::Text("Learning rate: %.6f", m_optimizer->learning_rate());
		...
		if (m_testbed_mode == ETestbedMode::Nerf && ImGui::TreeNode("NeRF training options")) {
			...
			ImGui::SliderFloat("Depth supervision strength", &m_nerf.training.depth_supervision_lambda, 0.f, 10.f);
...
void Testbed::mouse_wheel(Vector2f m, float delta) {
	if (delta == 0 || ImGui::GetIO().WantCaptureMouse) {
		return;
	}
...
void Testbed::init_window(int resw, int resh, bool hidden, bool second_window) {
	...
	std::string title = "Instant Graphics Avatars (";
	switch (m_testbed_mode) {
		case ETestbedMode::Image: title += "Image"; break;
		case ETestbedMode::Sdf: title += "SDF"; break;
		case ETestbedMode::Nerf: title += "INSTA"; break;
...
void Testbed::reset_network(bool clear_density_grid) {
    ...
    m_random_generator = default_rng_t{42};
	...
	if (m_testbed_mode == ETestbedMode::Nerf) {
		...
		json& mapper_network_config = config["mapper_network"];
        std::string nerf_network_type = config["nerf_network"];

		uint32_t n_dir_dims = 3;
		uint32_t n_extra_dims = m_nerf.training.dataset.n_extra_dims();

        if (nerf_network_type == "concatenation")
            m_network = m_nerf_network = std::make_shared<NerfNetwork<precision_t>>(
                dims.n_pos,
                n_dir_dims,
                n_extra_dims,
                dims.n_pos + 1 + 1 + 3, // The offsets from dt, holder(frame_id or depth), deformed_pos. HACKY
                encoding_config,
                dir_encoding_config,
                network_config,
                rgb_network_config,
                mapper_network_config
            );
        else {
            tlog::error() << "Wrong config nerf_network!";
            throw std::runtime_error("Wrong config nerf_network!");
        }
...
void Testbed::train(uint32_t batch_size) {
	...
	if (fast_mod(m_training_step, n_prep_to_skip) == 0) {
		auto start = std::chrono::steady_clock::now();
		ScopeGuard timing_guard{[&]() {
...
void Testbed::render_frame(const Matrix<float, 3, 4>& camera_matrix0, const Matrix<float, 3, 4>& camera_matrix1, const Vector4f& nerf_rolling_shutter, CudaRenderBuffer& render_buffer, bool to_srgb) {
	...
	switch (m_testbed_mode) {
		case ETestbedMode::Nerf:
			if (!m_render_ground_truth && !m_offscreen_rendering) {
            ...
            if (m_raycast_flame_mesh) {
                raycast_flame(render_buffer, max_res, focal_length, camera_matrix0, screen_center, m_stream.get());
            }
			break;
...
void Testbed::save_snapshot(const std::string& filepath_string, bool include_optimizer_state) {
	...
	std::ofstream f(filepath.str(), std::ios::out | std::ios::binary);
...
void Testbed::load_snapshot(const std::string& filepath_string) {
	...
	m_training_step = uint32_t(m_network_config["snapshot"]["training_step"]) + 1;
