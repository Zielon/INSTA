...
__global__ void mark_flame_density_grid(
    ...
    auto nn = closest_triangle(pos, bvhnodes, tris_bvh);
    int id = nn.first;
    float dist = nn.second;
    auto tri = tris_bvh[id];
//    printf("%f, %f, %f dist %f\n", pos.x(), pos.y(), pos.z(), dist);
    if (dist < 0.015 && tri.mask_id != 0) {
        grid_out[i] = 1.f;
    }
}
...
__global__ void generate_next_nerf_network_inputs(
	...
	Vector3f dir = payload.dir;
	Vector3f idir = dir.cwiseInverse();
    Vector3f center = render_bsphere.m_p;
	...
	for (uint32_t j = 0; j < n_steps; ++j) {
		Vector3f pos{};
		float dt = 0.0f;
        network_input(i + j * n_elements)->holder = -1.f;
		while (1) {
			pos = origin + dir * t;
            float sphere_dist = (pos - center).norm();

//            auto nn = closest_triangle(pos, bvhnodes[frameId], tris_bvh[frameId]);
//            int id = nn.first;
//            float dist = nn.second;
        ...
        network_input(i + j * n_elements)->holder = float(frameId);
		t += dt;
	}
...
__global__ void composite_kernel_nerf(
	...
	for (; j < actual_n_steps; ++j) {
		...
		Vector3f warped_deformed_pos = input->deformed_pos.p;
		Vector3f pos = unwarp_position(warped_pos, aabb);
		Vector3f deformed_pos = unwarp_position(warped_deformed_pos, aabb);
//        printf("[%f, %f, %f], [%f, %f, %f]\n", warped_pos.x(), warped_pos.y(), warped_pos.z(), warped_deformed_pos.x(), warped_deformed_pos.y(), warped_deformed_pos.z());
		...
		if (render_mode == ERenderMode::Normals) {
			...
			rgb = Array3f::Constant(cam_fwd.dot(deformed_pos - origin) * depth_scale);
		...
		if (weight > payload.max_weight) {
			...
			local_depth = cam_fwd.dot(deformed_pos - camera_matrix.col(3));
		}
...
//	Vector2f tminmax = render_sphere.ray_intersect(ray_unnormalized.o, ray_d_normalized);
...
__global__ void compute_loss_kernel_train_nerf(
	...
	float seg_mask_supervision_lambda,
	float near_distance,
    bool penalize_density,
    float beta
) {
	const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
	...
	for (; compacted_numsteps < numsteps; ++compacted_numsteps) {
		...
		const Vector3f pos = unwarp_position(coords_in.ptr->deformed_pos.p, aabb);
    ...
    int mask_id = -1;
    float seg_supervision = 1.f;
    float depth_loss_gradient = 0.f;

    if (metadata[img].seg_mask) {
        Array4i seg_mask_samp = read_seg_mask(xy, resolution, metadata[img].seg_mask, metadata[img].image_data_type);
        mask_id = seg_mask_samp[0];

        if (mask_id == 120 || mask_id == 130 || mask_id == 140) { seg_supervision = seg_mask_supervision_lambda; }
//        if (mask_id == 140) { depth_supervision_lambda = 0.25f; }
	...
	for (uint32_t j = 0; j < compacted_numsteps; ++j) {
		...
		const Vector3f pos = unwarp_position(coord_in->deformed_pos.p, aabb);
		...
		const float beta_supervision = dt / (T_ray - 1.f + 1e-3f);

		float dloss_by_dmlp = density_derivative * (
			dt * (lg.gradient.matrix().dot((T * rgb - suffix).matrix()) + depth_supervision) + beta * beta_supervision
		);
        ...
        if (penalize_density) {
            ...
            dloss_by_dmlp += (texsamp.w() <= 0.01f) ? mask_supervision_strength * weight : 0.f;
        }
...
void Testbed::NerfTracer::init_rays_from_camera(
	...
	const BoundingSphere& render_bshpere,
...
uint32_t Testbed::NerfTracer::trace(
	...
	while (i < MARCH_ITER) {
		...
		linear_kernel(generate_next_nerf_network_inputs, 0, stream,
			...
			cone_angle_constant,
            extra_dims_gpu,
            testbed.m_nerf.extra_dim_idx_for_inference,
            testbed.m_render_radius,
            testbed.m_bvh_ptrs_gpu.data(),
            testbed.m_triangles_ptrs_gpu.data()
		);
        ...
        if (testbed.m_render_deformed)
            positions_matrix = testbed.render_deformed(positions_matrix, stream);
...
void Testbed::Nerf::Training::reset_extra_dims(default_rng_t &rng) {
    ...
    if (extra_dims_gpu.data()) {
        CUDA_CHECK_THROW(cudaMemcpy(extra_dims_cpu.data(), extra_dims_gpu.data(), n_extra_dims * (dataset.n_images + 1) * sizeof(float), cudaMemcpyDeviceToHost));
    }
		...
		if (m_data_path.is_directory()) {
			for (const auto& path : fs::directory{m_data_path}) {
				if (path.is_file() && equals_case_insensitive(path.extension(), "json")) {
                    if (path.str().find("synthetic") != std::string::npos || path.str().find("train") != std::string::npos) {
...
void Testbed::update_density_grid_flame(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
    ...
    density_matrix = point_density_flame_closest_point(density_grid_position_matrix, density_matrix, m_nerf.training.dataset.density_mesh_distance, stream);
...
void Testbed::update_density_grid_nerf(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
    ...
    if (m_iso_surface_occ_grid)
        linear_kernel(mark_flame_density_grid, 0, stream, n_elements, m_nerf.density_grid.data(), m_canon_bvh_gpu, m_canon_tris_gpu);
	...
	for (uint32_t i = 0; i < n_steps; ++i) {
        ...
        density_grid_position_matrix = surface_closest_point(density_grid_position_matrix, stream);

		m_nerf_network->density(stream, density_grid_position_matrix, density_matrix, m_random_generator, false, m_nerf.training.extra_dims_gpu.data(), m_nerf.training.n_images_for_training);
...
void Testbed::train_nerf(uint32_t target_batch_size, bool get_loss_scalar, cudaStream_t stream) {
	...
	if (train_extra_dims) {
        ...
        std::vector<float> extra_dims_cpu(m_nerf.training.extra_dims_gpu.size());
		...
		m_nerf.training.extra_dims_gpu.copy_to_host(extra_dims_cpu);
		// Optimization step
		for (uint32_t i = 0; i < m_nerf.training.n_images_for_training; ++i) {
			...
			for (uint32_t j = 0; j < n_extra_dims; ++j) {
                extra_dims_new_values[i * n_extra_dims + j] = extra_dims_cpu[i * n_extra_dims + j];
                if (j >= int(n_extra_dims / 2))
...
void Testbed::train_nerf_step(uint32_t target_batch_size, uint32_t n_rays_per_batch, uint32_t* counter, uint32_t* compacted_counter, float* loss, cudaStream_t stream) {
    ...
    coords_matrix = surface_closest_point(coords_matrix, stream);
	...
	linear_kernel(compute_loss_kernel_train_nerf, 0, stream,
		...
		m_nerf.training.seg_mask_supervision_lambda,
		m_nerf.training.near_distance,
        m_nerf.training.penalize_density_alpha,
        m_nerf.training.beta
	);
...
//    if (m_training_step == 0) {
//        update_density_grid_flame(alpha, NERF_GRIDSIZE()*NERF_GRIDSIZE()*NERF_GRIDSIZE()*n_cascades, 0, stream);
//    }
}
...
void Testbed::optimise_mesh_step(uint32_t n_steps) {
	...
	for (uint32_t i = 0; i < n_steps; ++i) {
		...
		m_nerf_network->density(m_stream.get(), positions_matrix, density_matrix, m_random_generator);
